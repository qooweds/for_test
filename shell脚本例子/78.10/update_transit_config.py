...
def generate_logic_config():
    ...
    doc.root['is_transit_server'] = is_transit_server
    doc.root.log['level'] = log_level 
...
def generate_gate_config():
    ...
    doc.root.zone_server['port'] = gate_server_port
    doc.root.transit_server['ip'] = transit_server_ip
    doc.root.transit_server['port'] = transit_server_port
    doc.root.transit_server['is_open'] = is_transit_open
    ...
    if server_list.count(server_id) < 1  and server_list.count(str(server_id)) <1 :
    ...
    for id in server_list:
        server_id_list.append(BeautifulSoup(server_id_template % id, "html.parser"))
...
def generate_zone_config():
    ...
    doc.root['is_transit_server'] = is_transit_server
    doc.root.log['level'] = log_level 
    doc.root.gate_acceptor['listen_port'] = gate_server_port
...
if __name__ == "__main__" :
    ...
    try:
        ...
        is_transit_open = "false"
        transit_server_ip = "127.0.0.1"
        transit_server_port = "10"
