...
if [ "$1" == "stop" ] ;then
  ...
  ssh kingnet@$SERVER_IP -f "LD_LIBRARY_PATH=$REMOTE_GCC_PATH sh /$ARR_SERVER_PATH/server_ctl.sh $1" >>./log/$SERVER_ID.log
else
  ssh kingnet@$SERVER_IP "LD_LIBRARY_PATH=$REMOTE_GCC_PATH sh /$ARR_SERVER_PATH/server_ctl.sh $1"
fi
