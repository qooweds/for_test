...
UPDATE_DB_FILE="alter_table_20131126.sql"
...
REMOTE_GCC_PATH="/usr/local/lib;/opt/app/gcc-4.7.3/lib64"
...
#  fi
echo $SERVER_ID
  ssh kingnet@$SERVER_IP "/opt/app/mysql/bin/mysql -ushengxy -pdq32t61q -h$SERVER_DB_IP -P$DB_PORT $SERVER_DB_NAME -e \" select * from role_info limit 1;\" "
done
