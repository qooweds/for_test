...
do
  ...
  ssh kingnet@$SERVER_IP -f "LD_LIBRARY_PATH=$REMOTE_GCC_PATH sh /$ARR_SERVER_PATH/server_ctl.sh $1" 
  ssh kingnet@$SERVER_IP "/bin/bash /$ARR_SERVER_PATH/gm/gen_rank_xml.sh"
